...
const ABOUT_CONTENT = {
  ...
  closingParagraph:
    "A special resource bank has been created where all the research published, best practices, program code, module, designing, algorithm and leadership developed over the years has been made accessible to the startups. A highly qualified team is functioning day in and out for smooth functioning of the center, which is monitored periodically by the governing body.",
};
...
const CORE_AREAS = [
  ...
  },
  {
    ...
    link: "https://www.prithvi.ai/",
  },
  {
    title: "Internet Of Things",
    color: "bg-orange-500",
    img: "internet-of-things.png",
  },
  {
...
const CoreAreaCard = ({ card }) => {
  const CardBody = () => (
    <div
      className={`${card.color} h-full rounded-2xl p-5 shadow-lg text-center flex flex-col items-center justify-center transform hover:-translate-y-1.5 transition-all duration-300 group`}
    >
      ...
      <strong className="text-white text-sm md:text-base font-semibold block tracking-wide">
        {card.title}
      </strong>
    </div>
  );
    ...
    <div className="bg-[#012640] text-white rounded-2xl p-6 md:p-10 shadow-2xl relative overflow-hidden min-h-[300px] flex flex-col justify-between">
      <div className="transition-all duration-500 ease-in-out">
        ...
        <div className="text-center">
          ...
          <p className="text-slate-400 text-xs md:text-sm mt-1 whitespace-pre-line">
    ...
    <div className="text-slate-800 min-h-screen font-sans selection:bg-blue-500 selection:text-white">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="bg-white rounded-2xl p-6 md:p-10 shadow-xl border border-slate-100 transition-all duration-300">
          ...
          <div
            className={`overflow-hidden transition-all duration-700 ease-in-out ${isExpanded ? "max-h-[2500px] opacity-100" : "max-h-0 opacity-0"}`}
          >
            {ABOUT_CONTENT.expandedParagraphs.map((paragraph, index) => (
              <p
                key={index}
                className="text-base md:text-lg leading-relaxed text-slate-600 mb-6 pt-2"
              >
                {paragraph}
              </p>
            ))}
      ...
      <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 mb-8">
        <h2 className="text-2xl font-bold text-center text-slate-900 mb-10 uppercase tracking-widest">
          Messages
        </h2>
