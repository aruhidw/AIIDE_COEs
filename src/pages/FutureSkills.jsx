...
const FutureSkills = () => {
  ...
  // Clears active timers safely to prevent timing glitches on manual click overrides
  const resetTimer = () => {
  ...
  // Replicating bootstrap's data-bs-ride="carousel" behavior safely
  useEffect(() => {
    resetTimer();
  ...
  return (
    <div className="bg-transparent min-h-screen pb-20">
      ...
      {/* Main Container Wrapper mirroring 'container pb-5' */}
      <main className="max-w-5xl mx-auto px-4 py-12 bg-transparent">
        
        {/* Core Slide Frame Controller Layout Wrapper */}
        <div className="relative w-full overflow-hidden rounded-[15px] group shadow-2xl bg-slate-950">
          
          {/* Hardware-Accelerated Sliding Track (Bootstrap Fluid Slide Emulation) */}
          <div 
            ...
            {CAROUSEL_IMAGES.map((_, index) => (
              <button
                ...
                className={`h-1.5 transition-all duration-300 rounded-full ${
                  index === activeIndex ? 'bg-white w-6' : 'bg-white/40 hover:bg-white/70 w-1.5'
                }`}
        ...
        {/* Bottom CTA Action Button Section */}
